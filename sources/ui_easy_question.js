async function ui_easy_question() {
    ...
    button_E_answer = [];
    button_E_answer[0] = createButton('True').style('transform', 'translate(-100%,+50%)').style('width','150px').style('height','50px').position(windowWidth / 2, windowHeight / 2).class('buttons_style');
    button_E_answer[0].mouseClicked(easy_answer0_selected);
    button_E_answer[1] = createButton('False').style('transform', 'translate(0%,+50%)').style('width','150px').style('height','50px').position(windowWidth / 2, windowHeight / 2).class('buttons_style');
